...
const PLUGINS_DIR = path.join(process.cwd(), 'subzero');
describe('Plugin Loading', () => {
    let pluginFiles = [];
    const loadedPlugins = [];
    const errors = [];
    beforeAll(async () => {
        pluginFiles = fs.readdirSync(PLUGINS_DIR = path.join(process.cwd(), 'subzero');
        for (const file of pluginFiles) {
            try {
                const mod = await import(path.join(PLUGINS_DIR = path.join(process.cwd(), 'subzero');
    ...
    it('has plugins directory', () => expect(fs.existsSync(PLUGINS_DIR = path.join(process.cwd(), 'subzero');
    it('has more than 100 plugins', () => expect(pluginFiles.length).toBeGreaterThan(100));
    it('ALL plugins load without errors', () => {
        if (errors.length > 0)
            console.error('Failed plugins:', errors);
    ...
    it('aliases are arrays', () => {
        const invalid = loadedPlugins.filter(p => p.mod?.aliases !== undefined && !Array.isArray(p.mod.aliases));
    ...
    it('cooldown is a number if present', () => {
        const invalid = loadedPlugins.filter(p => p.mod?.cooldown !== undefined && typeof p.mod.cooldown !== 'number');
        expect(invalid.map(p => p.file)).toEqual([]);
    });
});
